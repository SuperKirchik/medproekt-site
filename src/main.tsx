...
const policySections = [
  ...
  [
    ...
    [
      "АО «ТаймВэб» (ИНН 7810353960) - размещение Сайта и серверной инфраструктуры; оператор корпоративной электронной почты - доставка и хранение заявок после его согласования Оператором; сервис Calltouch - телефония, коллтрекинг и учет обращений; ООО «Яндекс» - Яндекс.Метрика после согласия на аналитические cookie; работники и подрядчики Оператора с необходимым доступом и обязанностями по конфиденциальности.",
    ],
  ],
  [
  ...
  [
    ...
    [
      "7.1. Строго необходимые технологии используются для работы, безопасности, телефонии и учета обращений, включая Calltouch. Яндекс.Метрика загружается только после выбора пользователя в cookie-баннере.",
...
const consentParagraphs = [
  ...
  "4. Для указанных целей Оператор вправе поручить обработку АО «ТаймВэб» в части размещения сайта и инфраструктуры, согласованному оператору корпоративной электронной почты в части доставки и хранения заявок, а также сервису Calltouch в части телефонии, коллтрекинга и учета обращений. Эти лица обязаны обеспечивать конфиденциальность и безопасность данных.",
...
function CookieBanner() {
  ...
  const save = (allowAnalytics: boolean) => {
    ...
    if (allowAnalytics) {
      loadMetrika();
    } else if (document.querySelector("script[data-metrika]")) {
  ...
  return (
    ...
    >
      <div>
        <p>
          Мы используем необходимые технологии для работы сайта, телефонии и
          учета обращений, включая Calltouch. С вашего согласия мы также
          используем Яндекс.Метрику для аналитики посещений и источников
          обращений. Подробнее - в{" "}
